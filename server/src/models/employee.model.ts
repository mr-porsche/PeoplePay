...
import type { Employee, PaginatedEmployees, EmployeeFilters } from '@peoplepay/shared';
...
export class EmployeeModel {
  ...
  findAll(filters: EmployeeFilters = {}): PaginatedEmployees {
    ...
    if (search) {
      conditions.push('(full_name LIKE @search OR email LIKE @search OR job_title LIKE @search)');
    ...
    const total = (
      this.db.prepare(`SELECT COUNT(*) as count FROM employees ${where}`).get(params) as {
        count: number;
      }
    ).count;
  ...
  findById(id: number): Employee | undefined {
    return this.db.prepare('SELECT * FROM employees WHERE id = ?').get(id) as Employee | undefined;
  }

  findByEmail(email: string): Employee | undefined {
    return this.db.prepare('SELECT * FROM employees WHERE email = ?').get(email) as
      | Employee
      | undefined;
  }
  ...
  distinctCountries(): string[] {
    return (
      this.db
        .prepare("SELECT DISTINCT country FROM employees WHERE status = 'active' ORDER BY country")
