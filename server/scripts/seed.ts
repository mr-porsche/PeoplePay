...
function generateEmail(fullName: string, index: number): string {
  const clean = fullName
    ...
    .replace(/\s+/g, '.')
    .replace(/[^a-z.]/g, '');
  const domains = ['gmail.com', 'yahoo.com', 'outlook.com', 'company.io', 'work.co', 'corp.net'];
...
async function seed() {
  ...
  console.log(` Mode : ${WIPE_FIRST ? 'fresh (wipe + reseed)' : 'additive (skip if exists)'}\n`);
  ...
  if (existing > 0 && !WIPE_FIRST) {
    console.log(`⚠️ Database already has ${existing.toLocaleString()} employees.`);
  ...
  if (warned.length > 0) {
    console.log(`⚠️   ${warned.length} name(s) could not be fully paired — filled with 'Unknown'`);
  }
